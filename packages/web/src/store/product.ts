...
export const useProductStore = create<ProductStoreState>((set, get) => {
    ...
    return {
        ...
        setForm: (form) =>
            set((state) => {
                const nextForm = { ...state.form, ...form }
                ...
                return { form: nextForm, errors: nextErrors, isDirty: true }
            }),
        resetForm: () =>
            set({
                form: {
        ...
        updateVariant: (index, variant) =>
            set((state) => {
                ...
                variants[index] = { ...currentVariant, ...variant }

                const nextErrors = { ...state.errors }
                Object.keys(variant).forEach((key) => {
                    const errorKey = `variant-${index}-${key}`
                    if (nextErrors[errorKey]) {
                        delete nextErrors[errorKey]
                    }
                })

                return { form: { ...state.form, variants }, errors: nextErrors, isDirty: true }
            }),
        removeVariant: (index) =>
            set({
                form: {
                    ...get().form,
                    variants: get().form.variants?.filter((_, currentIndex) => currentIndex !== index),
                },
                isDirty: true,
            }),
        validate: () => {
            ...
            form.variants?.forEach((variant, index) => {
                if (!variant.brand?.trim()) {
                    errors[`variant-${index}-brand`] = 'Brand is required.'
                }
