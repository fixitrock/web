...
export interface CartStoreState {
    ...
    note: Record<PaymentMethodType, string>
...
export const useCartStore = create<CartStoreState>((set, get) => ({
    ...
    addItem: (newItem) =>
        set((state) => {
            const existingItem = state.items.find((item) => item.variant.id === newItem.variant.id)
        ...
        set((state) => ({
            items: state.items.map((item) =>
                item.id === id
                    ? {
                          ...item,
                          serialNumbers: item.serialNumbers.map((serial, i) =>
        ...
        set((state) => ({
            items: state.items.map((item) =>
                item.id === id
                    ? {
                          ...item,
                          serialNumbers: item.serialNumbers.filter((_, i) => i !== index),
                      }
                    : item
            ),
        })),
    ...
    canAddItem: (product, selectedOptions, qtyToAdd = 1) => {
        ...
        const variant = product.variants.find((item) => {
            const brandMatch = item.brand === selectedOptions.brand
            const colorMatch =
                !item.color?.name || !selectedOptions.color || item.color.name === selectedOptions.color
            const storageMatch =
                !item.storage || !selectedOptions.storage || item.storage === selectedOptions.storage
    ...
    order: () => {
        ...
        return {
            ...
            products: state.items.map((item) => ({
                ...
                image: item.selectedOptions.image || item.product.thumbnail || null,
