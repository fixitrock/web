...
DECLARE
  ...
  v_permissions text[];
BEGIN
  -- Step 1: Identify authenticated user
  v_auth_id := auth.uid();
...
CREATE OR REPLACE FUNCTION "public"."my_transactions"("p_seller_id" "uuid", "p_user_id" "uuid", "page" integer DEFAULT 1, "page_limit" integer DEFAULT 40) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
...
SELECT jsonb_build_object(
    ...
    'total', (SELECT count(*) FROM tx_base),

    'hasMore', (page * page_limit) < (SELECT count(*) FROM tx_base),

    'empty', (SELECT count(*) FROM tx_base) = 0,
...
FROM tx_page t
LEFT JOIN orders_joined o ON o.id = t."orderID"
LEFT JOIN products_agg p ON p.order_id = o.id;
...
ALTER FUNCTION "public"."my_transactions"("p_seller_id" "uuid", "p_user_id" "uuid", "page" integer, "page_limit" integer) OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."transactions"
    ADD CONSTRAINT "transactions_seller_fkey" FOREIGN KEY ("sellerID") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "Seller can insert only their own transactions" ON "public"."transactions" FOR INSERT WITH CHECK (("auth"."uid"() = "sellerID"));



CREATE POLICY "Seller can update only their own transactions" ON "public"."transactions" FOR UPDATE USING (("auth"."uid"() = "sellerID")) WITH CHECK (("auth"."uid"() = "sellerID"));



CREATE POLICY "Seller can view their own transactions" ON "public"."transactions" FOR SELECT USING (("auth"."uid"() = "sellerID"));
...
GRANT ALL ON FUNCTION "public"."my_transactions"("p_seller_id" "uuid", "p_user_id" "uuid", "page" integer, "page_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."my_transactions"("p_seller_id" "uuid", "p_user_id" "uuid", "page" integer, "page_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."my_transactions"("p_seller_id" "uuid", "p_user_id" "uuid", "page" integer, "page_limit" integer) TO "service_role";
