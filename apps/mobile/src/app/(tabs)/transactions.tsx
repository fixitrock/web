...
import type { BalanceSummary, MyTransaction, TransactionSummary } from '@/src/types/user'
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const load = async () => {
                ...
                setTransactions(
                    ((transactionsResult as { data?: { transactions?: MyTransaction[] } })?.data
                        ?.transactions ?? []) as MyTransaction[]
                )
                setSummary(
                    ((transactionsResult as { data?: { summary?: TransactionSummary } })?.data
                        ?.summary ?? null) as TransactionSummary | null
                )
                setLoading(false)
            }
        ...
        >
            <Card className='border border-black/8 bg-[#161616] shadow-none dark:border-white/10'>
                <Card.Body className='gap-2 p-5'>
                    <Text className='text-sm tracking-[1.8px] text-white/55 uppercase'>Handle</Text>
            ...
            {summary ? (
                <View className='flex-row gap-3'>
                    ...
                    <StatCard label='Received' value={formatCurrency(summary.totalReceived)} />
                </View>
            ) : null}
