...
const getMatchValue = (match: RegExpMatchArray | null, index: number) => match?.[index] ?? ''
...
export function detectDevice(userAgent: string): DeviceInfo {
    ...
    if (ua.includes('windows')) {
        ...
        osVersion = getMatchValue(match, 1).replace('_', '.')
        ...
        osVersion = getMatchValue(match, 1)
    ...
    if (ua.includes('chrome') && !ua.includes('edg')) {
        ...
        browserVersion = getMatchValue(match, 1)
    ...
    if (ua.includes('mobile') || ua.includes('android') || ua.includes('iphone')) {
        ...
        if (ua.includes('iphone')) {
            ...
            if (match) {
                const version = parseInt(getMatchValue(match, 1), 10)
