...
import { myOrders, sellerOrders, sellerRecentOrders, sellerTop, sellerTopCategories } from '@/actions/user'
...
export function useTopCategories(username: string) {
    const query = useQuery({
        queryKey: ['topCategories', username],
        queryFn: () => sellerTopCategories(username),
        staleTime: 1000 * 60 * 5,
        retry: 1,
        refetchOnWindowFocus: false,
    })
