...
import { MyOrderItem, Order, RecentOrder, ReturnData, TopItem, TopStats } from '@/types/orders'
...
export async function sellerTopCategories(username: string) {
    return unstable_cache(
        async () => {
            const supabase = await createStaticClient()

            const { data, error } = await supabase.rpc('seller_top_categories', {
                p_username: username,
            })

            if (error) {
                console.error('[sellerTopCategories] error:', error)
                return []
            }

            return (data ?? []) as TopItem[]
        },
        [`top-categories:${username}`],
        {
            tags: [`top-categories:${username}`],
            revalidate: 60,
        }
    )()
}
