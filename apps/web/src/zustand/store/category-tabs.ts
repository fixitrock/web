...
const updateClientMetadata = (category: string | null, search: string, page: number) => {
    ...
    const heading = category || (search ? `Search: ${search}` : page > 1 ? `Products Page ${page}` : 'Products')
...
const getProductsUrlState = () => {
    ...
    return {
        isProductsTab: true,
        hasCategory: Boolean(rawCategory?.trim()),
        category: normalizeStoreCategory(rawCategory),
    ...
    persist(
        ...
        }),
        {
            ...
            merge: (persistedState, currentState) => {
                const persisted = persistedState as Partial<CategoryTabsState> | undefined
