    ...
    return (
        <div className='space-y-2'>
            {transactions?.map((t) => {
                ...
                const displayType =
                    view === 'user' ? (t.type === 'credit' ? 'debit' : 'credit') : t.type
                const isDebit = displayType === 'debit'

                return (
                    <div
                        key={t.id}
                        className={`flex flex-col gap-0.5 ${isDebit ? 'items-end justify-end' : 'items-start justify-start'}`}
                    >
                        <div
                            aria-expanded={isExpanded}
                            className='min-w-72 overflow-hidden rounded-lg border'
                            role='button'
                            tabIndex={0}
                                ...
                                <div className='flex items-center justify-between gap-2'>
                                    <div className='flex min-w-0 flex-1 items-center gap-1.5'>
                                        ...
                                        >
                                            <Icon
                                                className={
                                                    isDebit ? 'text-red-700' : 'text-green-700'
                                                }
                                                icon={getIcon(t.note)}
                                            />
                                        </div>
