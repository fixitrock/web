...
export async function verifyFirebaseLogin(idToken: string): Promise<{ user?: User; error?: string }> {
    try {
        ...
        if (!userId) {
            ...
            if (newUser.user) {
            ...
            } else if (createError?.code === 'phone_exists' || createError?.message?.includes('registered')) {
                ...
                const zombie = users.find(u => u.phone?.replace(/\D/g, '')?.includes(targetSimple))
